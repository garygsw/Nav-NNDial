...
# For task reference tracker
class CNNReferenceTracker(BaseNNModule):

    def __init__(self, task_size, vocab_size, ihidden_size, ohidden_size):

        # source CNN encoder
        self.sCNN = CNNEncoder(vocab_size, ihidden_size,
                pool=(False,False,True), level=3)
        self.tCNN = CNNEncoder(vocab_size, ohidden_size,
                pool=(False,False,True), level=3)
    ...
    def value_recur(self, vsrcpos_jsv, vtarpos_jsv, ref_jm1v,
                    ref_jm1N, ngms_j, ngmt_jm1, uttms_j, uttmt_jm1):

        # source features
        vsrcemb_jsv = T.sum(ngms_j[vsrcpos_jsv,:],axis=0)
        src_jsv = T.concatenate([vsrcemb_jsv,uttms_j],axis=0)
        # target features
        vtaremb_jsv = T.sum(ngmt_jm1[vtarpos_jsv,:],axis=0)
        tar_jsv = T.concatenate([vtaremb_jsv,uttmt_jm1],axis=0)
        # update g_jv
    ...
    def recur(self, ref_jm1, ms_j, mt_jm1, mscut_j, mtcut_jm1,
        ...
        # new belief  : belief, ignore value for = none (the last one)
        g_j,_  = theano.scan(fn=self.value_recur,\
                sequences=[ vsrcpos_js, vtarpos_jm1s, ref_jm1[:-1]],
                non_sequences=[ ref_jm1[-1], ngms_j, ngmt_jm1, uttms_j, uttmt_jm1],
                outputs_info=None)
    ...
    def track(self, ref_jm1, ms_j, mt_jm1, vsrcpos_js, vtarpos_jm1s):
        ...
        for v in range(self.dbm1):
            ...
            src_jsv = np.concatenate([vsrcemb_jsv,uttms_j],axis=0)
            # target features
            vtaremb_jsv = np.sum(ngmt_jm1[vtarpos_jm1s[v],:],axis=0)
            tar_jsv = np.concatenate([vtaremb_jsv,uttmt_jm1],axis=0)

            # update g_jv value
            g_jv = np.dot( self.Whb_backup, sigmoid(
                np.dot(src_jsv,self.Wfbs_backup) +
                np.dot(tar_jsv,self.Wfbt_backup) +
                ref_jm1[v] * self.Wrec_backup +
                self.B0_backup ))
            g_j[v] = g_jv

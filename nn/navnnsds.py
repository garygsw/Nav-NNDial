    ...
    def config_theano(self):
        ...
        def dialog_recur(source_t, target_t, source_len_t, target_len_t,
            ...
            if self.dec=='lstm' and self.learn_mode!='trk':
                if self.ply == 'attention':
                    input_belief_t = belief_t + task_ref_t
                    # input_belief_t = T.concatenate([belief_t, task_ref_t], axis=1)
                else:
                ...
                if self.ply!='latent': # deterministic policy
                    ...
                    if self.learn_mode=='rl': # rl fine-tuning
                        ...
                        r_t =  G.disconnected_grad(
                            ...
                            -_lambdaf*T.sum(Q*(T.log10(Q+epsln)-T.log10(P+epsln)))+ # KL(P|Q)
                            -_lambdaf*T.sum(Qtm1*(T.log10(Qtm1+epsln)-T.log10(Q+epsln))) # KL(Qt|Qtm1)
                        )
        ...
        #theano.printing.pydotprint(self.train, outfile="pics/pre-complile.png", var_with_name_simple=True)
